...
export default function Layout({ children, activeTab = 'dashboard', setActiveTab }: LayoutProps) {
  const [sidebarOpen, setSidebarOpen] = useState(true);
  const [showNotifications, setShowNotifications] = useState(false);

  // Mock notification data - in real app this would come from a context or API
  const unreadNotifications = 3;
  const hasUrgentNotifications = true;
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600">
                {activeTab === 'dashboard' && 'Overview of all residents and activities'}
                {activeTab === 'residents' && 'Manage resident information and records'}
                {activeTab === 'reports' && 'View reports and analytics'}
                {activeTab === 'calendar' && 'Schedule appointments and events'}
                {activeTab === 'programs' && 'Manage programs and phases'}
                {activeTab === 'staff' && 'Staff management and assignments'}
