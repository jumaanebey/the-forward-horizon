...
import { acquisitionManager, Lead, MarketingCampaign, VirtualTour, FacilityShowcase, ConversionFunnel } from "../utils/acquisitionManager";
...
export default function AcquisitionHub({ isOpen, onClose }: AcquisitionHubProps) {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [leadsData, campaignsData, toursData, showcasesData, analyticsData] = await Promise.all([
        acquisitionManager.listLeads(),
        acquisitionManager.listCampaigns(),
        acquisitionManager.listVirtualTours(),
        acquisitionManager.listShowcases(),
        acquisitionManager.getAcquisitionAnalytics()
      ]);

      setLeads(leadsData);
      setCampaigns(campaignsData);
      setVirtualTours(toursData);
      setShowcases(showcasesData);
      setAnalytics(analyticsData);

      // Load funnel
      const funnels = await acquisitionManager.getFunnel('default');
      if (funnels) {
        const updatedFunnel = await acquisitionManager.updateFunnelMetrics('default');
  ...
  const handleCreateLead = async (leadData: any) => {
    try {
      const newLead = await acquisitionManager.createLead(leadData);
  ...
  const handleCreateCampaign = async (campaignData: any) => {
    try {
      const newCampaign = await acquisitionManager.createCampaign(campaignData);
  ...
  const handleUpdateLeadStatus = async (leadId: string, status: Lead['status'], notes?: string) => {
    try {
      const updatedLead = await acquisitionManager.updateLeadStatus(leadId, status, notes);
      if (updatedLead) {
            ...
            <div className="space-y-6">
              ...
              {loading ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
                  ...
                  <div className="bg-white p-6 rounded-lg border shadow-sm">
                    <h4 className="font-semibold text-gray-900 mb-4">Lead Status</h4>
                    <div className="space-y-3">
                      {Object.entries(analytics.leadStatuses).map(([status, count]) => (
          ...
          {activeTab === 'tours' && (
            <div className="space-y-6">
              ...
              {loading ? (
                <div className="text-center py-8">
                  <div className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div>
                  <p className="mt-2 text-gray-600">Loading virtual tours...</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {virtualTours.map((tour) => (
                    <div key={tour.id} className="bg-white p-6 rounded-lg border shadow-sm">
                      <div className="flex items-center justify-between mb-4">
                        <h4 className="font-semibold text-gray-900">{tour.title}</h4>
                        <span className="text-sm text-gray-500 capitalize">{tour.type.replace('_', ' ')}</span>
                      </div>
                      ...
                      <div className="space-y-3">
                        <div>
                          ...
                          <p className="text-sm font-medium text-gray-900">{tour.rooms.length} rooms</p>
                        </div>

                        <div>
                          <p className="text-sm text-gray-600">Amenities</p>
                          <p className="text-sm font-medium text-gray-900">{tour.amenities.length} amenities</p>
                        </div>

                        <div>
                          <p className="text-sm text-gray-600">Testimonials</p>
                          <p className="text-sm font-medium text-gray-900">{tour.testimonials.length} testimonials</p>
                        </div>
          ...
          {activeTab === 'showcase' && (
            <div className="space-y-6">
              ...
              {loading ? (
                <div className="text-center py-8">
                  <div className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div>
                  <p className="mt-2 text-gray-600">Loading showcases...</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {showcases.map((showcase) => (
                    <div key={showcase.id} className="bg-white p-6 rounded-lg border shadow-sm">
                      <h4 className="font-semibold text-gray-900 mb-4">{showcase.name}</h4>
                      ...
                      <div className="space-y-4">
                        <div>
                          ...
                          <ul className="text-sm text-gray-600 space-y-1">
                            {showcase.highlights.slice(0, 3).map((highlight, index) => (
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-2 gap-2 text-sm">
                            {showcase.availability.slice(0, 4).map((room, index) => (
                              <div key={index} className="flex justify-between">
                                <span className="text-gray-600">{room.roomType}:</span>
                                <span className="font-medium text-gray-900">{room.available}/{room.total}</span>
                              </div>
                            ))}
                          </div>
                        </div>
              ...
              {loading ? (
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div className="bg-white p-6 rounded-lg border shadow-sm">
                      <p className="text-sm font-medium text-gray-600">Total Leads</p>
                      <p className="text-2xl font-bold text-gray-900">{funnel.metrics.totalLeads}</p>
                    </div>
                    <div className="bg-white p-6 rounded-lg border shadow-sm">
                      <p className="text-sm font-medium text-gray-600">Qualified Leads</p>
                      <p className="text-2xl font-bold text-gray-900">{funnel.metrics.qualifiedLeads}</p>
                    </div>
                    <div className="bg-white p-6 rounded-lg border shadow-sm">
                      <p className="text-sm font-medium text-gray-600">Tours</p>
                      <p className="text-2xl font-bold text-gray-900">{funnel.metrics.tours}</p>
                    </div>
                    <div className="bg-white p-6 rounded-lg border shadow-sm">
                      <p className="text-sm font-medium text-gray-600">Conversions</p>
                      <p className="text-2xl font-bold text-gray-900">{funnel.metrics.conversions}</p>
                    </div>
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <select name="source" className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
